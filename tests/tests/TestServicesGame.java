...
class TestServicesGame {
	...
	void askForGameGivingCorrectNameAndPlatform() {
		...
		assertEquals(platform, game.getRelease().getPlatform());
	}
	...
	void askForGameGivingIncorrectNameCapsAndCorrectPlatform() {
		...
		assertNotEquals(name, game.getName());
		assertEquals(null, game.getRelease());
	...
	void askForGameGivingCorrectNameAndEmptyPlatform() {
		...
		assertThrows(IllegalArgumentException.class, () -> servicesGame.getGames(name, platform));
